...
c
c Fortran functions called by OriginalRosenbluthCollisionOperator.
c

      subroutine getRampOriginal(A, v, vra, vrb,
     *     vmin, vmax, solution_order)
c
...
c +++++++++++++
c
      subroutine getDOriginal(d1, d2, d3, vTh, vx, vy, vxgrid, vygrid,
...
c
c.. declarations of incoming variables
      real d1, d2, d3, vTh, vx, vy, vxgrid, vygrid
...
c
      eps = 1.e-10
      call getRampOriginal(Ax, vxgrid, range_lo(1), range_hi(1),
     *                     vxmin, vxmax, solution_order)
      call getRampOriginal(Ay, vygrid, range_lo(2), range_hi(2),
     *                     vymin, vymax, solution_order)
      vx2 = vx*vx
      vy2 = vy*vy
      v  = sqrt(vx2+vy2)
      v = v/vTh
...
c     other velocities use the explicit expression for D.
      if (v .lt. 0.01) then
        factor = Ax*Ay*vTh*vTh*sqrt2divpi
...
c ++++++++++++++
c
      subroutine computeOriginalRosenbluthDiffusionTensor(
     *     d,
...
c
c.. declarations of local variables
      integer i1, i2, i3, i4, solution_order, nghosts, do_relativity
...
c
      ...
      do i4 = n4a-nghosts, n4b+nghosts
        do i3 = n3a-nghosts, n3b+nghosts
          ...
          do i2 = n2a, n2b
            do i1 = n1a, n1b
              call getDOriginal(d1, d2, d3, vTh, vx, vy, vxgrid, vygrid,
     *                          range_lo, range_hi, vxmin, vxmax,
     *                          vymin, vymax, solution_order,
     *                          sqrt2divpi)
              d(i1,i2,i3,i4,1) = d1
              d(i1,i2,i3,i4,2) = d2
...
c +++++++++++++
c
      subroutine computeOriginalRosenbluthTemps(
     *     fM, uMomx, uMomy, uKE,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     velocities,
     *     dparams)
...
c +++++++++++++
c
      subroutine computeOriginalRosenbluthDenoms(
     *     rMomx, rMomy, rKE,
     *     cMomx, cMomy, cKE,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
...
c +++++++++++++
c
      subroutine computeOriginalRosenbluthNumerators(
     *     rMomx, rMomy, rKE,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthBackReaction(
     *     rhs,
     *     cMomx, cMomy, cKE,
     *     IMomxN, IMomyN, IKEN,
     *     IMomxD, IMomyD, IKED,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision(
     *     rhs,f,velocities,D,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c
      if( solution_order .eq. 4 ) then
        ...
        if (kernel_alg .eq. 0) then
          call appendOriginalRosenbluthCollision_4thMaplePrimitive(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
          ...
          call appendOriginalRosenbluthCollision_4thMapleLog(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
          ...
          call appendOriginalRosenbluthCollision_4thMapleOrig(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
          ...
          call appendOriginalRosenbluthCollision_4thMapleAsinh(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
        ...
        ! 6th order
        if (kernel_alg .eq. 0) then
          call appendOriginalRosenbluthCollision_6thMaplePrimitive(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
          ...
          call appendOriginalRosenbluthCollision_6thMapleLog(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
          ...
          call appendOriginalRosenbluthCollision_6thMapleOrig(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
     ...
     *       iparams)
          ...
          call appendOriginalRosenbluthCollision_6thMapleAsinh(
     *       rhs,f,velocities,D,
     *       nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *       n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *       dx,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_4thMaplePrimitive(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
     *     dparams,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_4thMapleLog(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_4thMapleOrig(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c            alpha(i,j) = 1.0 / (dparams(4) * (Vth(i1,i2)**2))
        ...
        t2 = vx(1,0) ** 2
        t4 = vy(1,0) ** 2
        t8 = exp(-(t2 + t4) * alpha / 0.2E1)
        t9 = t8 * nu
        t10 = D(1,0,2)
        t15 = vx(1,1) ** 2
        t17 = vy(1,1) ** 2
        t21 = exp(-(t15 + t17) * alpha / 0.2E1)
        t23 = 0.1E1 / t21 * f(1,1)
        t26 = 0.1E1 / t8 * f(1,0)
        t28 = 0.1E1 / dvy
        t29 = t28 * (t23 - t26)
        t33 = vx(1,-1) ** 2
        t35 = vy(1,-1) ** 2
        t39 = exp(-(t33 + t35) * alpha / 0.2E1)
        t41 = 0.1E1 / t39 * f(1,-1)
        t43 = t28 * (t26 - t41)
        t47 = (t29 / 0.2E1 + t43 / 0.2E1) * t10 * t9
        t49 = vx(0,0) ** 2
        t51 = vy(0,0) ** 2
        t55 = exp(-(t49 + t51) * alpha / 0.2E1)
        t56 = t55 * nu
        t57 = D(0,0,2)
        t60 = vx(0,1) ** 2
        t62 = vy(0,1) ** 2
        t66 = exp(-(t60 + t62) * alpha / 0.2E1)
        t68 = 0.1E1 / t66 * f(0,1)
        t71 = 0.1E1 / t55 * f(0,0)
        t72 = t68 - t71
        t73 = t28 * t72
        t76 = vx(0,-1) ** 2
        t78 = vy(0,-1) ** 2
        t82 = exp(-(t76 + t78) * alpha / 0.2E1)
        t84 = 0.1E1 / t82 * f(0,-1)
        t85 = t71 - t84
        t86 = t28 * t85
        t90 = (t73 / 0.2E1 + t86 / 0.2E1) * t57 * t56
        t92 = 0.1E1 / dvx
        t93 = t92 * (t47 - t90)
        t96 = vx(-1,0) ** 2
        t98 = vy(-1,0) ** 2
        t102 = exp(-(t96 + t98) * alpha / 0.2E1)
        t103 = t102 * nu
        t104 = D(-1,0,2)
        t108 = vx(-1,1) ** 2
        t110 = vy(-1,1) ** 2
        t114 = exp(-(t108 + t110) * alpha / 0.2E1)
        t116 = 0.1E1 / t114 * f(-1,1)
        t119 = 0.1E1 / t102 * f(-1,0)
        t121 = t28 * (t116 - t119)
        t124 = vx(-1,-1) ** 2
        t126 = vy(-1,-1) ** 2
        t130 = exp(-(t124 + t126) * alpha / 0.2E1)
        t132 = 0.1E1 / t130 * f(-1,-1)
        t134 = t28 * (t119 - t132)
        t138 = (t121 / 0.2E1 + t134 / 0.2E1) * t104 * t103
        t140 = t92 * (t90 - t138)
        t142 = dvx ** 2
        t144 = 0.1E1 / t142 * dvy
        t148 = vx(1,2) ** 2
        t150 = vy(1,2) ** 2
        t154 = exp(-(t148 + t150) * alpha / 0.2E1)
        t156 = 0.1E1 / t154 * f(1,2)
        t162 = (t29 - t43) * t144
        t169 = vx(1,-2) ** 2
        t171 = vy(1,-2) ** 2
        t175 = exp(-(t169 + t171) * alpha / 0.2E1)
        t177 = 0.1E1 / t175 * f(1,-2)
        t190 = vx(2,0) ** 2
        t192 = vy(2,0) ** 2
        t196 = exp(-(t190 + t192) * alpha / 0.2E1)
        t197 = t196 * nu
        t202 = vx(2,1) ** 2
        t204 = vy(2,1) ** 2
        t208 = exp(-(t202 + t204) * alpha / 0.2E1)
        t210 = 0.1E1 / t208 * f(2,1)
        t213 = 0.1E1 / t196 * f(2,0)
        t218 = vx(2,-1) ** 2
        t220 = vy(2,-1) ** 2
        t224 = exp(-(t218 + t220) * alpha / 0.2E1)
        t226 = 0.1E1 / t224 * f(2,-1)
        t240 = vx(0,2) ** 2
        t242 = vy(0,2) ** 2
        t246 = exp(-(t240 + t242) * alpha / 0.2E1)
        t248 = 0.1E1 / t246 * f(0,2)
        t249 = t248 - t68
        t254 = (t73 - t86) * t144
        t255 = -(t249 * t28 - t73) * t144 + t254
        t260 = vx(0,-2) ** 2
        t262 = vy(0,-2) ** 2
        t266 = exp(-(t260 + t262) * alpha / 0.2E1)
        t268 = 0.1E1 / t266 * f(0,-2)
        t269 = t84 - t268
        t273 = -t254 + (-t269 * t28 + t86) * t144
        t279 = (t28 * t255 / 0.12E2 + t28 * t273 / 0.12E2) * t57 * t56
        t282 = t92 * (t93 - t140) / 0.6E1
        t287 = vx(-1,2) ** 2
        t289 = vy(-1,2) ** 2
        t293 = exp(-(t287 + t289) * alpha / 0.2E1)
        t295 = 0.1E1 / t293 * f(-1,2)
        t301 = (t121 - t134) * t144
        t307 = vx(-1,-2) ** 2
        t309 = vy(-1,-2) ** 2
        t313 = exp(-(t307 + t309) * alpha / 0.2E1)
        t315 = 0.1E1 / t313 * f(-1,-2)
        t328 = vx(-2,0) ** 2
        t330 = vy(-2,0) ** 2
        t334 = exp(-(t328 + t330) * alpha / 0.2E1)
        t335 = t334 * nu
        t340 = vx(-2,1) ** 2
        t342 = vy(-2,1) ** 2
        t346 = exp(-(t340 + t342) * alpha / 0.2E1)
        t348 = 0.1E1 / t346 * f(-2,1)
        t351 = 0.1E1 / t334 * f(-2,0)
        t356 = vx(-2,-1) ** 2
        t358 = vy(-2,-1) ** 2
        t362 = exp(-(t356 + t358) * alpha / 0.2E1)
        t364 = 0.1E1 / t362 * f(-2,-1)
        t382 = D(1,0,1) * t9
        t385 = D(2,0,1) * t197
        t389 = D(0,0,1) * t56
        t391 = t92 * (t382 - t389)
        ...
        t397 = D(-1,0,1) * t103
        t399 = t92 * (t389 - t397)
        t402 = (t391 - t399) * dvx / 0.16E2
        t404 = t26 - t71
        t409 = D(-2,0,1) * t335
        t416 = t71 - t119
        t422 = t382 / 0.2E1 + t389 / 0.2E1
        t423 = t213 - t26
        t425 = t92 * t404
        t428 = t92 * t416
        t430 = t92 * (t425 - t428)
        t431 = -t92 * (t423 * t92 - t425) + t430
        t440 = t92 * t404 * t422
        t444 = t389 / 0.2E1 + t397 / 0.2E1
        t446 = t92 * t416 * t444
        t448 = t92 * (t440 - t446)
        t452 = t119 - t351
        t456 = -t430 + t92 * (-t452 * t92 + t428)
        t471 = t66 * nu
        t473 = D(0,1,3) * t471
        t475 = t246 * nu
        t477 = D(0,2,3) * t475
        ...
        t490 = D(0,-1,3) * t488
        t492 = t28 * (t481 - t490)
        t495 = (t483 - t492) * dvy / 0.16E2
        t500 = t266 * nu
        t502 = D(0,-2,3) * t500
        t514 = t473 / 0.2E1 + t481 / 0.2E1
        t523 = t28 * t72 * t514
        t527 = t481 / 0.2E1 + t490 / 0.2E1
        t529 = t28 * t85 * t527
        t531 = t28 * (t523 - t529)
        t550 = D(0,1,2)
        t552 = t92 * (t23 - t68)
        t554 = t92 * (t68 - t116)
        t558 = (t552 / 0.2E1 + t554 / 0.2E1) * t550 * t471
        t562 = (t425 / 0.2E1 + t428 / 0.2E1) * t57 * t56
        t564 = t28 * (t558 - t562)
        t566 = D(0,-1,2)
        t568 = t92 * (t41 - t84)
        t570 = t92 * (t84 - t132)
        t574 = (t568 / 0.2E1 + t570 / 0.2E1) * t566 * t488
        t576 = t28 * (t562 - t574)
        t583 = t92 * (t552 - t554)
        t619 = (t92 * t431 / 0.12E2 + t92 * t456 / 0.12E2) * t57 * t56
        t622 = (t564 - t576) * t144 / 0.6E1
        t630 = t92 * (t568 - t570)
        cg = t93 / 0.2E1 + t140 / 0.2E1 + (t92 * ((t28 * (-(t28 * (t156 
     #- t23) - t29) * t144 / 0.6E1 + t162 / 0.6E1) / 0.2E1 + t28 * (-t16
     #2 / 0.6E1 + (t43 - t28 * (t41 - t177)) * t144 / 0.6E1) / 0.2E1) * 
     #t10 * t9 - t92 * (t92 * ((t28 * (t210 - t213) / 0.2E1 + t28 * (t21
     #3 - t226) / 0.2E1) * D(2,0,2) * t197 - t47) - t93) / 0.6E1 - t279 
     #+ t282) / 0.2E1 + t92 * (t279 - t282 - (t28 * (-(t28 * (t295 - t11
     #6) - t121) * t144 / 0.6E1 + t301 / 0.6E1) / 0.2E1 + t28 * (-t301 /
     # 0.6E1 + (t134 - t28 * (t132 - t315)) * t144 / 0.6E1) / 0.2E1) * t
     #104 * t103 + t92 * (t140 - t92 * (t138 - (t28 * (t348 - t351) / 0.
     #2E1 + t28 * (t351 - t364) / 0.2E1) * D(-2,0,2) * t335)) / 0.6E1) /
     # 0.2E1) * t142 + t92 * (t92 * t404 * (t382 / 0.2E1 - (t92 * (t385 
     #- t382) - t391) * dvx / 0.16E2 + t395 - t402) - t92 * t416 * (t395
     # - t402 + t397 / 0.2E1 - (t399 - t92 * (t397 - t409)) * dvx / 0.16
     #E2)) + (t92 * t431 * t422 / 0.24E2 - t92 * (t92 * (t92 * t423 * (t
     #385 / 0.2E1 + t382 / 0.2E1) - t440) - t448) / 0.24E2 - t92 * t456 
     #* t444 / 0.24E2 + t92 * (t448 - t92 * (t446 - t92 * t452 * (t397 /
     # 0.2E1 + t409 / 0.2E1))) / 0.24E2) * dvx + t28 * (t28 * t72 * (t47
     #3 / 0.2E1 - (t28 * (t477 - t473) - t483) * dvy / 0.16E2 + t487 - t
     #495) - t28 * t85 * (t487 - t495 + t490 / 0.2E1 - (t492 - t28 * (t4
     #90 - t502)) * dvy / 0.16E2)) + t28 * (t28 * t255 * t514 / 0.24E2 -
     # (t28 * (t28 * t249 * (t477 / 0.2E1 + t473 / 0.2E1) - t523) - t531
     #) * t144 / 0.24E2 - t28 * t273 * t527 / 0.24E2 + (t531 - t28 * (t5
     #29 - t28 * t269 * (t490 / 0.2E1 + t502 / 0.2E1))) * t144 / 0.24E2)
     # * t142 + t564 / 0.2E1 + t576 / 0.2E1 + (t28 * ((t92 * (-t92 * (t9
     #2 * (t210 - t23) - t552) / 0.6E1 + t583 / 0.6E1) / 0.2E1 + t92 * (
     #-t583 / 0.6E1 + t92 * (t554 - t92 * (t116 - t348)) / 0.6E1) / 0.2E
     #1) * t550 * t471 - (t28 * ((t92 * (t156 - t248) / 0.2E1 + t92 * (t
     #248 - t295) / 0.2E1) * D(0,2,2) * t475 - t558) - t564) * t144 / 0.
     #6E1 - t619 + t622) / 0.2E1 + t28 * (t619 - t622 - (t92 * (-t92 * (
     #t92 * (t226 - t41) - t568) / 0.6E1 + t630 / 0.6E1) / 0.2E1 + t92 *
     # (-t630 / 0.6E1 + t92 * (t570 - t92 * (t132 - t364)) / 0.6E1) / 0.
     #2E1) * t566 * t488 + (t576 - t28 * (t574 - (t92 * (t177 - t268) / 
     #0.2E1 + t92 * (t268 - t315) / 0.2E1) * D(0,-2,2) * t500)) * t144 /
     # 0.6E1) / 0.2E1) * t142
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_4thMapleAsinh(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_6thMaplePrimitive(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_6thMapleLog(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_6thMapleOrig(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
...
c +++++++++++++
c
      subroutine appendOriginalRosenbluthCollision_6thMapleAsinh(
     *     rhs,fin,velocities,din,
     *     nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     *     n1a,n1b,n2a,n2b,n3a,n3b,n4a,n4b,
     *     dx,
