...
#include "TwoSpecies_ElectronTZSourceF.H"
#include "Loki_Utilities.H"

namespace Loki {

const string TwoSpecies_ElectronTrigTZSource::s_CLASS_NAME("TwoSpecies_ElectronTrigTZSource");

bool
TwoSpecies_ElectronTrigTZSource::isType(
   const string& a_name)
{
...
TwoSpecies_ElectronTrigTZSource::TwoSpecies_ElectronTrigTZSource(
...
void
TwoSpecies_ElectronTrigTZSource::set(
   ParallelArray& a_dist_func,
   const ProblemDomain& a_domain,
   double a_time,
   const ParallelArray& a_velocities) const
{
   FORT_SET_TWOELECTRON_TRIG_TZ_SOURCE(*a_dist_func.getData(),
...
void
TwoSpecies_ElectronTrigTZSource::computeError(
   ...
   const ParallelArray& a_velocities) const
{
   FORT_COMPUTE_TWOELECTRON_TRIG_TZ_SOURCE_ERROR(*a_tz_error.getData(),
      *a_dist_func.getData(),
...
void
TwoSpecies_ElectronTrigTZSource::printParameters() const
{
